...
const createSubDistributor = async function (req, res) {
    try {
      ...
      let checkSubDistributorName = await subDistributorModel.findOne({ subDistributorName: subDistributorName });
      if (checkSubDistributorName != null && checkSubDistributorName != undefined) {
  ...
  const getSubDistributorData = async function (req, res) {
    try {
      ...
      if (subDistributorId) {
        const getNewsubDistributor = await subDistributorModel.findOne({ _id: subDistributorId, banned:false });
      ...
      const allSubDistributorData = await subDistributorModel.find({banned:false});
...
const updateSubDistributorData = async function (req, res) {
    try {
      ...
      if(!subDistributorId){
        return res.status(400).send({ status: false, message: "subDistributorId is required" });
      }
      if (!mongoose.isValidObjectId(subDistributorId)) {
        return res.status(400).send({ status: false, message: "Invalid ID" });
      }
  
      let getUsubDistributor = await subDistributorModel.findById({_id:subDistributorId});
      ...
      if (adminId) {
        ...
        if (createdBy === "admin" && getUsubDistributor.adminId.toString() === adminId) {
          console.log(checkadmin, "================");
      ...
      if (distributorId) {
        console.log("inside subdistributor");
      ...
      if (!updatedSubDistributor) {
        return res.status(404).json({ status: false, message: "SubDistributor not found" });
      }
  ...
  module.exports = {createSubDistributor, getSubDistributorData, updateSubDistributorData}